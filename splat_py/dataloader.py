...
class ColmapData(GaussianSplattingDataset):
    ...
    def __init__(
        ...
        # num_points= np.random.randint(1000)
        # print("show sparse points",num_points)  
        ...
        for _, point in sparse_points.items():
            ...
            #     point.rgb / 255.0, dtype=torch.float32
            # )
            # self.xyz[row] = torch.tensor(np.random.rand(), dtype=torch.float32)
            self.rgb[row] = torch.tensor(
                    np.random.randint(255) / 255.0, dtype=torch.float32
                )
            row += 1 # / 0.28209479177387814
        ...
        print("load cameras",cameras.items())
        self.cameras = None
